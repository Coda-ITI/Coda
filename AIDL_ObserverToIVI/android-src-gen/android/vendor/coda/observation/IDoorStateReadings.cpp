...
static binder_status_t _aidl_android_vendor_coda_observation_IDoorStateReadings_onTransact(AIBinder* _aidl_binder, transaction_code_t _aidl_code, const AParcel* _aidl_in, AParcel* _aidl_out) {
  (void)_aidl_in;
  (void)_aidl_out;
  ...
  switch (_aidl_code) {
    case (FIRST_CALL_TRANSACTION + 0 /*onDoorStateChanged*/): {
      ...
      _aidl_ret_status = ::ndk::AParcel_readData(_aidl_in, &in_isOpen);
      if (_aidl_ret_status != STATUS_OK) break;

      ::ndk::ScopedAStatus _aidl_status = _aidl_impl->onDoorStateChanged(in_position, in_isOpen);
...
static const char* _g_aidl_android_vendor_coda_observation_IDoorStateReadings_clazz_code_to_function[] = { "onDoorStateChanged",};
static AIBinder_Class* _g_aidl_android_vendor_coda_observation_IDoorStateReadings_clazz = ::ndk::ICInterface::defineClass(IDoorStateReadings::descriptor, _aidl_android_vendor_coda_observation_IDoorStateReadings_onTransact, _g_aidl_android_vendor_coda_observation_IDoorStateReadings_clazz_code_to_function, 1);
...
::ndk::ScopedAStatus BpDoorStateReadings::onDoorStateChanged(int32_t in_position, bool in_isOpen) {
  ...
  _aidl_ret_status = ::ndk::AParcel_writeData(_aidl_in.get(), in_isOpen);
  if (_aidl_ret_status != STATUS_OK) goto _aidl_error;
  ...
  if (_aidl_ret_status == STATUS_UNKNOWN_TRANSACTION && IDoorStateReadings::getDefaultImpl()) {
    _aidl_status = IDoorStateReadings::getDefaultImpl()->onDoorStateChanged(in_position, in_isOpen);
...
::ndk::SpAIBinder BnDoorStateReadings::createBinder() {
  AIBinder* binder = AIBinder_new(_g_aidl_android_vendor_coda_observation_IDoorStateReadings_clazz, static_cast<void*>(this));
  #ifdef BINDER_STABILITY_SUPPORT
...
const char* IDoorStateReadings::descriptor = "android.vendor.coda.observation.IDoorStateReadings";
...
std::shared_ptr<IDoorStateReadings> IDoorStateReadings::fromBinder(const ::ndk::SpAIBinder& binder) {
  if (!AIBinder_associateClass(binder.get(), _g_aidl_android_vendor_coda_observation_IDoorStateReadings_clazz)) {
...
::ndk::ScopedAStatus IDoorStateReadingsDefault::onDoorStateChanged(int32_t /*in_position*/, bool /*in_isOpen*/) {
  ::ndk::ScopedAStatus _aidl_status;
